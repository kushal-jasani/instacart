...
const { getMainCategories, getAllStores, checkInStorePrices, getNextDeliveryTime, getStoresOpenAfterEleven, getPickupAvailableStores, getStoreByCategory } = require("../repository/store");
...
exports.getStoresByCategory=async(req,res,next)=>{
    try{
        ...
        const singleResData=[]
        const responseData = await Promise.all(stores.map(async (store) => {
